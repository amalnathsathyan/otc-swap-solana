...
function isAnchorError(
  error: any
): error is { error: any; errorLogs: string[] } {
  return (
    error &&
    typeof error === "object" &&
    "error" in error &&
...
describe("swap program - create offer", () => {
  ...
  before(async () => {
    try {
      ...
      const makerAirdropTx = await connection.requestAirdrop(
        maker.publicKey,
        airdropAmount
      );
      const maker2AirdropTx = await connection.requestAirdrop(
        maker2.publicKey,
        airdropAmount
      );
      const takerAirdropTx = await connection.requestAirdrop(
        taker.publicKey,
        airdropAmount
      );
      ...
      const tokenMetadata2  : TokenMetadata = {
        updateAuthority: admin.publicKey,
        mint: mint_b.publicKey,
        name: "THE TOKEN IS NOT RUSTY",
        symbol: "RUSTY2",
        uri: "https://dweb.link/ipfs/Qma5VS5zQ6fGusmkDME5J68MvS9MvFMV44zkxWatUmqiQ2", // URI to a richer metadata
        additionalMetadata: [["", ""]],
      };
      ...
      const metadataLen1 =
          TYPE_SIZE + LENGTH_SIZE + pack(tokenMetadata1).length;
      const mintLamports1 = await connection.getMinimumBalanceForRentExemption(
          mintLen + metadataLen1
      );
      const metadataLen2 =
          TYPE_SIZE + LENGTH_SIZE + pack(tokenMetadata2).length;
      const mintLamports2 = await connection.getMinimumBalanceForRentExemption(
          mintLen + metadataLen2
      );
        // Prepare transaction
      const inputMintTransaction = new Transaction().add(
          SystemProgram.createAccount({
            fromPubkey: maker.publicKey,
            newAccountPubkey: mint_a.publicKey,
            space: mintLen,
            lamports: mintLamports1,
          ...
          createInitializeInstruction({
            ...
            uri: tokenMetadata1.uri,
          }),
          createUpdateFieldInstruction({
            programId: TOKEN_2022_PROGRAM_ID,
            metadata: mint_a.publicKey,
            updateAuthority: maker.publicKey,
            field: tokenMetadata1.additionalMetadata[0][0],
            value: tokenMetadata1.additionalMetadata[0][1],
          })
        );
        // Initialize Token with metadata
      const inputMintTxSig = await sendAndConfirmTransaction(
          connection,
          inputMintTransaction,
          [maker, mint_a, maker]
        );

      console.log("Input mint created, signature:", inputMintTxSig )
        // Create associated token account
      const inputMintMakerATA = await createAssociatedTokenAccountIdempotent(
          connection,
          maker,
          mint_a.publicKey,
          maker.publicKey,
      ...
      console.log("Token Minted To InputMakerATA", mintSig);
      console.log("input mint:",await getTokenMetadata(connection, mint_a.publicKey));
      const inputMint = mint_a.publicKey;
      console.log("InputMint:", inputMint.toBase58());

      //creating ATA
      const inputMintMaker2ATA = await createAssociatedTokenAccountIdempotent(
        connection,
        maker2,
        mint_a.publicKey,
        maker2.publicKey,
      ...
      makerTokenAccount = inputMintMaker2ATA;

      console.log(
        "inputMint ATA for Maker Created:",
        inputMintMaker2ATA.toBase58()
      );

      //minting tokens mint_a to Maker2
      // Mint Token to associated token account
      ...
      console.log("Token Minted To InputMaker2ATA", mintSig2);


      ///taker output mint
      const outputMintTransaction = new Transaction().add(
        SystemProgram.createAccount({
          fromPubkey: taker.publicKey,
          newAccountPubkey: mint_b.publicKey,
          space: mintLen,
          lamports: mintLamports2,
          programId: TOKEN_2022_PROGRAM_ID,
        }),
        createInitializeMetadataPointerInstruction(
          mint_b.publicKey,
        ...
        createInitializeInstruction({
          programId: TOKEN_2022_PROGRAM_ID,
          metadata: mint_b.publicKey,
          updateAuthority: taker.publicKey,
          mint: mint_b.publicKey,
          mintAuthority: taker.publicKey,
          name: tokenMetadata2.name,
          symbol: tokenMetadata2.symbol,
          uri: tokenMetadata2.uri,
        }),
        createUpdateFieldInstruction({
          programId: TOKEN_2022_PROGRAM_ID,
          metadata: mint_b.publicKey,
          updateAuthority: taker.publicKey,
          field: tokenMetadata2.additionalMetadata[0][0],
          value: tokenMetadata2.additionalMetadata[0][1],
        })
      );
      // Initialize Token with metadata
    const outputMintTxSig = await sendAndConfirmTransaction(
        connection,
        outputMintTransaction,
        [taker, mint_b]
      );

    console.log("Input mint created, signature:", outputMintTxSig )
      // Create associated token account
    const outputMintTakerATA = await createAssociatedTokenAccountIdempotent(
        connection,
        maker,
        mint_b.publicKey,
        maker.publicKey,
      ...
      // Mint Token to associated token account
      const outputMintSig = await mintTo(
        connection,
        taker,
        mint_b.publicKey,
        outputMintTakerATA,
    ...
    console.log("Token Minted To InputMakerATA", outputMintSig);
    console.log("input mint:",await getTokenMetadata(connection, mint_b.publicKey));
    const outputMint = mint_b.publicKey;
    console.log("InputMint:", outputMint.toBase58());
  ...
  it("initialize admin", async () => {
    ...
    const feeWallet = new PublicKey(
      "2vBAnVajtqmP4RBm8Vw5gzYEy3XCT9Mf1NBeQ2TPkiVF"
    );
    ...
    const adminConfigFetched = await program.account.adminConfig.fetch(
      adminConfig
    );
    assert(
      adminConfigFetched.admin.equals(admin.publicKey),
    ...
    assert(
      feeConfigFetched.feeAddress.equals(feeWallet),
      "Fee wallet mismatch"
    );
    assert(
      feeConfigFetched.feePercentage.eq(feePercentage),
      "Fee percentage mismatch"
    );

    // Fetch and assert whitelistConfig state
    const whitelistConfigFetched = await program.account.whitelistConfig.fetch(
      whitelistConfig
    );
    assert(
      whitelistConfigFetched.requireWhitelist === requireWhitelist,
      "Whitelist requirement mismatch"
    );

    // Fetch and assert mintWhitelist state
    const mintWhitelistFetched = await program.account.mintWhitelist.fetch(
      mintWhitelist
    );
    assert.deepEqual(
    ...
    const [expectedAdminConfig, adminConfigBump] =
  ...
  it("add token-mints to whitelist", async () => {
    ...
    await program.methods
      .addMintsToWhitelist([mint_c.publicKey])
      .accounts({
        admin: admin.publicKey,
      })
      .signers([admin])
  ...
  it("removes token-mints from whitelist", async () => {
    ...
    await program.methods
      .removeMintsFromWhitelist([mint_c.publicKey])
      .accounts({
        admin: admin.publicKey,
      })
      .signers([admin])
  ...
  it("Fail: updated fee percentage is more than 100%", async () => {
    try {
      await program.methods
        .updateFeePercentage(new anchor.BN("10001"))
        .accounts({
          admin: admin.publicKey,
        })
        .signers([admin])
      ...
      if (isAnchorError(err)) {
        ...
        console.log(
          "Assertion passed: updated fee percentage is more than 100%"
        );
      } else {
        throw err; // Rethrow if it's not an Anchor error
      }
  ...
  it("Fail: updates fee percentage by non-admin user", async () => {
    try {
      ...
      if (isAnchorError(err)) {
        ...
        assert.strictEqual(err.error.errorMessage, "Unauthorized admin");
        console.log(
          "Assertion passed: updates fee percentage by non-admin user"
        );
      } else {
        throw err; // Rethrow if it's not an Anchor error
      }
  ...
  it("updates fee address", async () => {
    const newFeeWallet = new PublicKey(
      "B5WFNofBtPcFUS9oR2oAuxTHsSCUVp3C4VjFtejKEUnv"
    );
    await program.methods
      .updateFeeAddress(
    ...
    const feeConfigFetched = await program.account.feeConfig.fetch(feeConfig);
    assert(
      feeConfigFetched.feeAddress.equals(newFeeWallet),
      "Fee wallet mismatch"
    );
  ...
  it("Toggles check for token whitelist", async () => {
    const whitelistConfigFetchedPreCall =
      await program.account.whitelistConfig.fetch(whitelistConfig);
  ...
  it("Fail: Toggles check for token whitelist by non-admin user", async () => {
    try {
      ...
      if (isAnchorError(err)) {
        ...
        assert.strictEqual(err.error.errorMessage, "Unauthorized admin");
        console.log(
          "Assertion passed: Toggles check for token whitelist by non-admin user"
        );
      } else {
        throw err; // Rethrow if it's not an Anchor error
      }
  ...
  it("creates an offer and sends tokens to the vault", async () => {
    ...
    try {
      ...
      console.log(
        "Vault token account found or created:",
      ...
      console.log(
        "makerBalanceBefore:",
        Number(makerBalanceBefore.value.amount)
      );
    ...
    try {
      ...
      const offer1 = PublicKey.findProgramAddressSync(
        [
          Buffer.from("offer"),
          maker2.publicKey.toBuffer(),
          Buffer.from(new anchor.BN(offer1Id).toArray("le", 8)),
        ],
        program.programId
      )[0];
      ...
      const offer1TxSig = await sendAndConfirmTransaction(
        connection,
        offer1Tx,
        [maker2],
      ...
      console.log(
        "Offer1 created and tokens sent to vault successfully!",
        offer1TxSig
      );
      ...
      console.error(
        "Error creating offer1 and sending tokens to vault:",
        error
      );
    }
    //offer2
    try {
      ...
      const offer2 = PublicKey.findProgramAddressSync(
        [
          Buffer.from("offer"),
          maker2.publicKey.toBuffer(),
          Buffer.from(new anchor.BN(offer2Id).toArray("le", 8)),
        ],
        program.programId
      )[0];
      ...
      // Fetch maker's token balance before the transaction
      const maker2BalanceBefore = await connection.getTokenAccountBalance(
        makerTokenAccount,
        "confirmed"
      );
      console.log(
        "maker2BalanceBefore:",
        Number(maker2BalanceBefore.value.amount)
      );
      // const vaultBalanceBefore = await connection.getTokenAccountBalance(vaultTokenAccount,'confirmed');
      // console.log('vaultBalanceBefore:', Number(vaultBalanceBefore.value.amount))
      ...
      const offer2TxSig = await sendAndConfirmTransaction(
        connection,
        offer2Tx,
        [maker2],
        { commitment: "confirmed" }
      ...
      console.log(
        "Offer2 created and tokens sent to vault successfully!",
        offer2TxSig
      );

      // Fetch balances after the transaction
      const maker2BalanceAfter = await connection.getTokenAccountBalance(
        makerTokenAccount
      );
      console.log(
        "maker2BalanceAfter:",
      ...
      console.error(
        "Error creating offer2 and sending tokens to vault:",
        error
      );
    }
  });
  ...
  it("add taker addresses to whitelist by maker of the offer", async () => {
    ...
    await program.methods
      .manageWhitelist([
        taker.publicKey,
        new PublicKey("4sijvjMmXG8sdzwsLJevQ1dSXpY6a9T7fPRkVLeqr6Wm"),
    ...
    const takerWhitelistFetched = await program.account.whitelist.fetch(
      whitelist
    );
    const newWhitelist = [
      taker.publicKey,
  ...
  it("taker fully accepts an offer and swap tokens", async () => {
    ...
    try {
      ...
      if (vaultInfo) {
        ...
        const vaultAccountInfo = await connection.getParsedAccountInfo(
          vaultTokenAccount
        );
      ...
      console.log(
        ...
        offerAccount.tokenAmountRemaining.toString()
      );
    } catch (err) {
    ...
    console.log(
      "takerBalanceTokenBBefore:",
      Number(takerBalanceTokenBBefore.value.amount)
    );

    try {
      const takeOfferTx = await program.methods
        .takeOffer(new anchor.BN("500"))
        .accountsPartial({
          core: {
  ...
  it("maker cancels the offer and close vault&whitelist PDAs", async () => {
    try {
      ...
      const makerATokenBalanceBefore = await connection.getTokenAccountBalance(
        makerTokenAccount,
        "confirmed"
      );
      console.log("makerATokenAccount", makerTokenAccount);
      console.log(
        "makerATokenBalanceBefore",
        Number(makerATokenBalanceBefore.value.amount)
      );

      const vaultTokenAccountNow = await getOrCreateAssociatedTokenAccount(
        connection,
        maker2,
        mint_a.publicKey,
        offer,
      ...
      console.log(
        "vaultTokenAccount:",
        vaultTokenAccountNow.address.toBase58()
      );
      ...
      try {
        ...
        const cancelTxSig = await sendAndConfirmTransaction(
          connection,
          cancelTx,
          [maker2],
          { commitment: "confirmed" }
      ...
      try {
        const vaultStatusAfter = await connection.getAccountInfo(
          vaultTokenAccountNow.address
        );
      ...
      try {
        ...
        assert(
          offerStatusAfter.status,
          "Offer should not be closed but cancelled"
        );
      } catch (err) {
        throw err;
      }
  ...
  it("taker partially accepts an offer and maker cancels remaining", async () => {
    ...
    try {
      ...
      await program.methods
        .manageWhitelist([taker.publicKey])
        .accounts({
          maker: maker2.publicKey,
          offer: offer,
        })
      ...
      const takerTokenBBefore = await connection.getTokenAccountBalance(
        takerPaymentTokenAccount
      );
      const takerTokenABefore = await connection.getTokenAccountBalance(
      ...
      console.log(
        "Taker token B balance:",
        Number(takerTokenBBefore.value.amount)
      );
      console.log(
        "Initial taker token A balance:",
        Number(takerTokenABefore.value.amount)
      );
      ...
      console.log("\nPost-take state:");
      const vaultBalanceAfterTake = await connection.getTokenAccountBalance(
        vaultTokenAccount
      );
      const takerTokenAAfterTake = await connection.getTokenAccountBalance(
        takerReceiveTokenAccount
      );
      const offerAfterTake = await program.account.offer.fetch(offer);

      console.log(
        "Vault balance after take:",
        Number(vaultBalanceAfterTake.value.amount)
      );
      console.log(
        "Taker token A balance after take:",
        Number(takerTokenAAfterTake.value.amount)
      );
      console.log(
        "Remaining amount:",
        offerAfterTake.tokenAmountRemaining.toString()
      );
      ...
      try {
        ...
        const offerAccountAfterCancel = await program.account.offer.fetch(
          offer
        );

        // Verify the offer status is set to Cancelled
        assert.equal(
          JSON.stringify(offerAccountAfterCancel.status),
          JSON.stringify({ cancelled: {} }),
          "Offer status should be Cancelled"
        );

        // Verify other offer account fields are preserved
        assert.equal(
          offerAccountAfterCancel.offerId.toString(),
          offerIdPartial.toString(),
          "Offer ID should be preserved"
        );
        assert.equal(
          offerAccountAfterCancel.maker.toBase58(),
          maker.publicKey.toBase58(),
          "Maker public key should be preserved"
        );
